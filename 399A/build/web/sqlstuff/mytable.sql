...
CREATE TABLE `ECPresentations` (
	...
	'Ballet','Recital','Concert','Choral','Contemporary Dance','Comedy','Children') NOT NULL,
...
CREATE TABLE `ECPerformances` (
	...
	PRIMARY KEY (`pid`),
	INDEX `fk_ECPerformances_1_idx` (`showid` ASC),
...
CREATE TABLE `ECInfo` (
	...
	INDEX `fk_ECInfo_1_idx` (`showid` ASC),
	CONSTRAINT `fk_ECInfo_1`
	FOREIGN KEY (`showid`)
	REFERENCES `ECPresentations` (`mykey`)
	ON DELETE CASCADE
	ON UPDATE NO ACTION);
...
insert into ecuserroles values ('victor','Xiaoxiong LiZhao', MD5('sa123'),'clerical');
insert into ecuserroles values ('littlemad','mad davenport', MD5('a321'),'events');
